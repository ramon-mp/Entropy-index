...
def entropy_index(parcel, block):
    '''
    Function that calculates the entropy index for each block taking into 
    account the variety of urban land use of each cadastral parcels present 
    in the blocks.
    '''  
    ...
    for use in list_uses:
        gdf_prop['LN_' + use] = gdf_prop[use].apply(lambda x: log(x) if x != 0 else 0)
    ...
    gdf_entropy = gdf_prop[['MASA', 'NUMERADOR', 'USO']].groupby('MASA')\
        .agg({'NUMERADOR': 'sum', 'USO': 'nunique'}).reset_index()
    
    # calculate LN for the metric formula LN(k) - DENOMINADOR
    gdf_entropy['DENOMINADOR'] = gdf_entropy['USO'].apply(lambda x: log(x))
    ...
    gdf_entropy = gdf_entropy.replace([inf, -inf], 0)
